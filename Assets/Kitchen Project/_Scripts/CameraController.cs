...
public class CameraController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
        {
            ...
            {              
                Transform searchTransform = raycastHit.collider.transform;
                while (searchTransform = searchTransform.parent)
                {
                    if (searchTransform.tag == "Kitchen")
                    {
                        selectedObj = searchTransform;
                        break;
